...
let application = {
  create: async function (params, cb) {
    ...
    let pwbdEligibilty = "no";
    // TODO check PwBD criteria
    if (createData.pwbd == "yes" && createData.pwbdPercentage >= 40) {
      pwbdEligibilty = "yes";
    }
    ...
    if (createData.advId == config.form3_id) {
      if (
        pwbdEligibilty == "yes" ||
        createData.recruitMode != "Direct" ||
        createData.isRetired == "Yes"
      ) {
        applicationfee = 0;
      } else {
        if (
          createData.category == "UR" ||
          createData.category == "OBC" ||
          createData.category == "EWS"
        ) {
          applicationfee = 2360;
      ...
      if (pwbdEligibilty == "yes") {
        applicationfee = 0;
      } else {
        if (createData.category == "UR" || createData.category == "OBC") {
          applicationfee = 2360;
        } else if (
          createData.category == "SC" ||
          createData.category == "ST" ||
          createData.category == "EWS"
        ) {
          applicationfee = 1180;
        }
